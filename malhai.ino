...
const char *service_name = "free";
const char *pop = "arsh4588";
...
static uint8_t  mal= 21;
boolean mal_state_prev     = LOW;  // current state
boolean mal_state_new     = LOW;  // previous state

boolean BUZZER_STATE        = false;
unsigned long buzzer_timer  = 0;

char device1[] = "SecuritySwitch";
char device2[]= "Malfunction";
static Switch SecuritySwitch(device1, NULL);
static Switch Malfunction(device2, NULL);
MalfunctionS = false ;
...
void write_callback(Device *device, Param *param, const param_val_t val, void *priv_data, write_ctx_t *ctx) {
    ...
    if(strcmp(device_name, device2) == 0) {
        Serial.printf("Malfunction = %s\n", val.val.b? "true" : "false");
        
        if(strcmp(param_name, "Power") == 0) {
            Serial.printf("Received value = %s for %s - %s\n", 
                            val.val.b? "true" : "false", device_name, param_name);
            MalfunctionS = val.val.b;
            param->updateAndReport(val);
            
            if(MalfunctionS == true){
                esp_rmaker_raise_alert("Malfunction protection is on");
            }
            else{
                esp_rmaker_raise_alert("Malfunction protection is on");
            }
...
void arduinodata(){
  bool dataa = mal.getvalue();
  if(dataa == true){
     Serial.println("Malfunction detected!");
            esp_rmaker_raise_alert("Malfunction Alert!\nMalfunction is detected.");
  }
}
